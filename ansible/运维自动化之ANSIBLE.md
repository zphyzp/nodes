...
#在ansible服务器上执行
ssh-keygen
ssh-copy-id 192.168.123.7(被管理主机ip)
```
...
#Copy：从主控端复制文件到远程主机
      ...
      > ansible appserver -m copy -a 'src=/root/selinux dest=/etc/selinux/config backup=yes'
...
#File：设置文件属性
    ...
    > ansible websrvs -m file -a 'path=/app/test.txt state=absent'      删除文件
...
#Cron：计划任务
    ...
    创建任务   
     ...
     > ansible appserver -m cron -a 'minute=* weekday=2,4,6 job="/usr/bin/wall FBI WARNRING" name=cron-test'
    删除任务
...
#Yum：管理包
    ...
    ansible websrvs -m yum -a 'name=httpd state=present disable_gpg_check=yes'  忽略gpg检查
    可以同时安装多个程序包
...
#User：管理用户
    ...
    #name：用户名 system：指定为系统用户 shell：无法登录 home：家目录 uid：uid groups：从组 comment：描述 
    ansible appserver -m user -a 'name=naginx system=yes shell=/sbin/nologin  home=/data/nginx/ uid=10086 groups=root,bin comment="this is nginx service"'
...
- hosts: websrvs
  ...
  tasks:
    ...
    - name: Install configure file
      copy: src=files/httpd.conf dest=/etc/httpd/conf/
      notify: restart httpd          #与下面的handlers的name对应
    - name: ensure apache is running
      service: name=httpd state=started enabled=yes
  
  handlers:
    - name: restart httpd            #与上面的notfiy对应
...
ansible-playbook –t install,conf httpd.yml   #指定执行install,conf 两个标签运行
```
...
```shell
#变量的优先级：命令行指定>playbook文件指定>host文件中的
...
[root@localhost ansible]# ansible appserver -m shell -a 'ls -l /data/'
192.168.123.7 | CHANGED | rc=0 >>
-rw------- 1 root root 0 Mar 22 14:15 www82.zptest.com.log
192.168.123.6 | CHANGED | rc=0 >>
-rw------- 1 root root 0 Mar 22 14:15 www81.zptest.com.log

#ansible_fqdn是ansible -m setup模块中取主机名的变量
```
...
```nginx
user  nginx;
worker_processes  {{ ansible_processor_vcpus*2 }};    #定义进程数量为主机cpu数x2
...
http {
    ...
    server {
          listen       {{ http_port }} default_server;       #定义监听端口号为http_port参数
...
- hosts: appserver
  ...
  tasks:
    - name: copy package
      copy: src=/root/nginx.rpm dest=/root/
    - name: install nginx
      yum: name=/root/nginx.rpm
    - name: copy template
...
- hosts: appserver
  ...
  tasks:
    ...
    - name: install nginx
      yum: name=/root/nginx.rpm
    - name: copy template for7
      template: src=nginx.conf7.j2 dest=/etc/nginx/nginx.conf backup=yes
      when: ansible_distribution_major_version == "7"   #当该系统变量为7时则使用该template
      notify: restart service
    - name: copy template for6
      template: src=nginx.conf6.j2 dest=/etc/nginx/nginx.conf backup=yes
      when: ansible_distribution_major_version == "6"   #当该系统变量为6时则使用该template
      notify: restart service
    - name: start service
      service: name=nginx state=started enabled=yes
...
```shell
[root@localhost ansible]# ansible-playbook testtemplate.yml 
PLAY [appserver] ***************************************************************
...
ok: [192.168.123.7]
ok: [192.168.123.6]
TASK [copy template for7] ******************************************************
changed: [192.168.123.6]
changed: [192.168.123.7]
TASK [copy template for6] **因为我的两台机器都是centos7，所以跳过centos6模板复制的步骤**
skipping: [192.168.123.6]
skipping: [192.168.123.7]
TASK [start service] ***********************************************************
ok: [192.168.123.7]
ok: [192.168.123.6]
...
- hosts: appserver
  ...
  vars:
    ports:                              #字典表为ports
      - web1:                                        
        port: 81						#对应模板#1	  
        name: www81.zptest.com          #对应模板#2
        rootdir: /data/website1             #对应模板#3
      - web2:
        port: 82
        name: www82.zptest.com
        rootdir: /data/website2

  tasks:
    - name: copy conf
      template: src=for1.conf.j2 dest=/data/for1.conf       #对应下方模板

#编辑一个模板与上面对应
$ vi for1.conf.j2

{% for p in ports%}              #嵌套上方ports字典表
server{
    listen {{p.port }}           #1
    servername {{p.name}}         #2
    documentroot {{p.rootdir}}     #3
}
{% endfor %}

#输出结果
[root@www82 data]# more for1.conf 
server{
    listen 81
    servername www81.zptest.com
    documentroot /data/website1
}
server{
    listen 82
    servername www82.zptest.com
...
{% if p.name is defined %}     #如果name被定义则创建servername
    servername {{p.name}}
{% endif %}
...
server{
    listen 81
    documentroot /data/website1           #未生成servername
}
server{
    listen 82
    servername www82.zptest.com
