...
#OS_Version=$(awk '{print $(NF-1)}' /etc/issue)

# declare script version date
#Script_Version="2019.05.05"


# define polling log path
LOGPATH=/var/log/check
[ -d $LOGPATH ] || mkdir -p $LOGPATH
RESULTFILE="$LOGPATH/1.163-Check-`hostname`-`date +%Y%m%d`.txt"
...
function getCpuStatus(){
    ...
    CPU_Arch=$(uname -m)
    S1=`vmstat 1 2|tail -1 | awk '{print $15}'`
    S2=`vmstat 1 2|tail -1 | awk '{print $15}'`
    S3=`vmstat 1 2|tail -1 | awk '{print $15}'`
    S4=`vmstat 1 2|tail -1 | awk '{print $15}'`
    S5=`vmstat 1 2|tail -1 | awk '{print $15}'`
    CPU_STAT=$((($S1+$S2+$S3+$S4+$S5)/5))
    echo  "CPU剩余:$CPU_STAT%"
